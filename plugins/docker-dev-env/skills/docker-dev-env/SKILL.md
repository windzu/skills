...
3. **Create docker-compose.yml**:
   ...
   - Mount `.claude-data/` for Claude Code data persistence
...
├── .claude-data/     # Claude Code data persistence (created by init-env.sh)
│   ├── dot-claude/   # Maps to ~/.claude inside container
│   └── claude.json   # Maps to ~/.claude.json inside container
├── .dockerignore
├── Dockerfile
├── docker-compose.yml
├── init-env.sh
...
# First time in container: authenticate Claude Code
claude
```
...
5. **Claude Code pre-installed** - Every container includes Claude Code, data persisted in `.claude-data/`
